...
int32_t conectar_socket(int descriptor, char* ipServer, uint32_t portServer) {
	int exit = 0;
	struct sockaddr_in direccion;
	direccion.sin_family = AF_INET;/*el el tipo de coneccion, igual que el primer parametro de socket().*/
...
int32_t enviar_paquete(uint32_t descriptor, void *mensaje, uint32_t length) {
	...
	while (aux < length) {
		...
		bytes_enviados = send(descriptor, mensaje + aux, length-aux, 0);
...
int32_t recibir(uint32_t descriptor, void *mensaje_recibido, uint16_t length) {
	...
	while (aux < length) {
		bytes_recibidos = recv(descriptor, mensaje_recibido + aux, length-aux, 0);
		aux += bytes_recibidos;
	}
	return aux;
}
